...
class Slack < BaseHandler
  ...
  def message
    ...
    message_fields = [
      ...
      {
        "title" => "Check",
        "value" => dashboard_link.nil? ? "#{check_name}" : "<#{dashboard_link}|#{check_name}>",
        "short" => true
      },
      {
        "title" => "Status",
    ...
    compact_msg = {
        "attachments" => [{
            ...
            "text"     => description(maxlen=400).sub(check_name, dashboard_link.nil? ? "#{check_name}" : "<#{dashboard_link}|#{check_name}>")
        }]
    }
